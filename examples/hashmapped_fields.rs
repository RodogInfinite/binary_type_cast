...
struct DataRecord {
    fields: HashMap<String, DataTypesCast>,
}
...
fn parse_records(record_descs: RecordDescs, data: &mut &[u8]) -> DataRecord {
    ...
    for record_description in record_descs.record_desc {
        ...
        let (tail, mut bytes) = parse_field_bytes(data, &record_description.length).unwrap();

        match record_description.number {
            1 => {
                data_record.set_field(
                    record_description.name.as_str(),
                    data_type.parse(&mut bytes),
                );
            }
            2 => {
                data_record.set_field(
...
fn main() {
    ...
    let data_record = parse_records(record_descs, &mut data);
    ...
    for (name, value) in &data_record.fields {
        ...
        match value {
            ...
            DataTypesCast::ASCIIString(val) => println!("\t{:<15}:{:?}", formatted_name, val),
        }
    }
    ...
    let data_types_IEEE754LSBSingleArr2_from_str = DataTypes::from_str("IEEE754LSBSingleArr2");
    println!(
        "\t{:<44}:\t{:?}",
    ...
    let data_types_IEEE754LSBDoubleArr2_from_str = DataTypes::from_str("IEEE754LSBDoubleArr2");
    println!(
        "\t{:<44}:\t{:?}",
    ...
    let data_types_IEEE754MSBSingleArr2_from_str = DataTypes::from_str("IEEE754MSBSingleArr2");
    println!(
        "\t{:<44}:\t{:?}",
    ...
    let data_types_IEEE754MSBDoubleArr2_from_str = DataTypes::from_str("IEEE754MSBDoubleArr2");
    println!(
        "\t{:<44}:\t{:?}",
    ...
    let data_types_IEEE754LSBSingleArr3_from_str = DataTypes::from_str("IEEE754LSBSingleArr3");
    println!(
        "\t{:<44}:\t{:?}",
    ...
    let data_types_IEEE754LSBDoubleArr3_from_str = DataTypes::from_str("IEEE754LSBDoubleArr3");
    println!(
        "\t{:<44}:\t{:?}",
