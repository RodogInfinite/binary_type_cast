...
use crate::utils::{cast_extraction::get_cast_types,CastTypeData, try_into::{build_type_variants_map, generate_try_into_impls}};
...
pub fn derive_macro(input: TokenStream) -> TokenStream {
    ...
    let errors = if let syn::Data::Enum(data_enum) = ast.data {
        ...
        return syn::Error::new_spanned(
            &ast,
            "TypeCast can only be derived for enums",
        )
        .to_compile_error()
        .into();
    };
    ...
    let type_variants_map = build_type_variants_map(
        &cast_type_data
    );
